...
export const handleGetNotifications: RequestHandler = (req: AuthRequest, res) => {
  try {
    if (!req.user) {
...
export const handleDeleteNotification: RequestHandler = (req: AuthRequest, res) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'Authentication required' });
    }
...
export const handleTestNotification: RequestHandler = (req: AuthRequest, res) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'Authentication required' });
    }
